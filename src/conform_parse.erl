...
unescape_single_quotes([$\\,$'|Rest]) ->
    [$"|unescape_single_quotes(Rest)];
unescape_single_quotes([]) -> [];
unescape_single_quotes([C|Rest]) ->
    [C|unescape_single_quotes(Rest)].
...
'setting'(Input, Index) ->
  p(Input, Index, 'setting', fun(I,D) -> (p_seq([p_zero_or_more(fun 'ws'/2), fun 'key'/2, p_zero_or_more(fun 'ws'/2), p_string(<<"=">>), p_zero_or_more(fun 'ws'/2), p_one_or_more(p_seq([p_choose([fun 'list_value'/2, fun 'single_quote_value'/2, fun 'double_quote_value'/2, fun 'value'/2]), p_optional(p_choose([p_string(<<",\s">>), p_string(<<",">>)]))])), p_zero_or_more(fun 'ws'/2), p_optional(fun 'comment'/2)]))(I,D) end, fun(Node, _Idx) ->
...
'key'(Input, Index) ->
  p(Input, Index, 'key', fun(I,D) -> (p_seq([p_label('head', p_choose([fun 'word'/2, p_choose([fun 'single_quote_value'/2, fun 'double_quote_value'/2])])), p_label('tail', p_zero_or_more(p_seq([p_string(<<".">>), p_choose([fun 'word'/2, p_choose([fun 'single_quote_value'/2, fun 'double_quote_value'/2])])])))]))(I,D) end, fun(Node, _Idx) ->
    [{head, H}, {tail, T}] = Node,
...
'list_value'(Input, Index) ->
  p(Input, Index, 'list_value', fun(I,D) -> (p_seq([p_not(p_choose([p_seq([p_zero_or_more(fun 'ws'/2), fun 'crlf'/2]), fun 'comment'/2])), p_string(<<"[">>), p_seq([p_not(p_string(<<"[">>)), p_one_or_more(p_seq([fun 'word'/2, p_zero_or_more(fun 'ws'/2), p_string(<<"=">>), p_zero_or_more(fun 'ws'/2), p_seq([p_choose([fun 'single_quote_value'/2, fun 'double_quote_value'/2, fun 'value_in_list'/2]), p_optional(p_choose([p_string(<<",\s">>), p_string(<<",">>)]))])]))]), p_string(<<"]">>)]))(I,D) end, fun(Node, _Idx) ->
...
-spec 'single_quote_value'(input(), index()) -> parse_result().
'single_quote_value'(Input, Index) ->
  p(Input, Index, 'single_quote_value', fun(I,D) -> (p_one_or_more(p_seq([p_not(p_choose([p_seq([p_zero_or_more(fun 'ws'/2), fun 'crlf'/2]), fun 'comment'/2])), p_string(<<"\\'">>), p_zero_or_more(p_choose([p_string(<<"\\\\">>), p_string(<<"\\\\'">>), p_seq([p_not(p_string(<<"\\'">>)), p_anything()])])), p_optional(p_seq([p_optional(p_string(<<"\r">>)), p_string(<<"\n">>)])), p_string(<<"\\'">>)])))(I,D) end, fun(Node, Idx) ->
